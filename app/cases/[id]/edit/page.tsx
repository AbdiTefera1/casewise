...
import { useUpdateCase } from '@/hooks/useCases'; // Import the useUpdateCase hook
...
export default function UpdateCasePage({ params }: { params: Promise<{ id: string }> }) {
  const { id: caseId } = use(params);
    ...
    const updateCaseMutation = useUpdateCase(); // Using the custom hook for updating

    const onSubmit = (data: CaseFormData) => {
        const { id, ...restData } = data;
        updateCaseMutation.mutate({ id: caseId, ...restData }, {
            onSuccess: () => {
                router.push('/cases');
            },
            onError: (error) => {
